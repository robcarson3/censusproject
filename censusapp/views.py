...
from .utils import (
	convert_year_range, 
	title_sort_key, issue_sort_key, copy_sort_key, 
...
def search(request, field=None, value=None, order=None):
	...
	elif order == '{{ COPY_ID_PREFIX }}':
		result_list = sorted(result_list, key=copy_sort_key)
...
def info(request, viewname):
	...
	estc_copy_count = models.Copy.objects.filter(from_estc=True).count()
	non_estc_copy_count = models.Copy.objects.filter(from_estc=False).count()
	facsimile_copy_count = models.Copy.objects.filter(
		~Q(digital_facsimile_url__in=[None, ''])
	).count()
	if copy_count > 0:
	...
	try:
		...
		formatted_content = page.content.format(**context)
		context = {
...
def provenance_name_copy_count_csv_export(request):
	...
	writer = csv.writer(response)
	writer.writerow(['Provenance Name', 'Bio', 'Number of Copies'])

	for entry in ownerships:
		owner = models.ProvenanceName.objects.get(pk=entry['owner'])
		writer.writerow([owner.name, owner.bio, entry['total']])
