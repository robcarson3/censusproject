...
def copy_location_sort_key(c):
	name = c.location.name if c.location else ''
	return strip_article(name)

def copy_shelfmark_sort_key(c):
	return c.shelfmark or ''

def copy_census_id_sort_key(c):
	census_id = c.census_id or ''
	try:    
		if '.' in census_id:
...
def get_collection(copy_list, collection_name):
	...
	feature_map = {
		...
		'unverified': {
			'filter': copy_list.filter(Q(verification='U')),
...
def census_globals(request):
	return {
		...
		'COPY_ID_PREFIX': getattr(settings, 'COPY_ID_PREFIX', ''),
